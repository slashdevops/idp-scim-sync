...
func TestNewDefaultConf(t *testing.T) {
	t.Run("using access key from env vars", func(t *testing.T) {
		...
		if Expected["AWS_ACCESS_KEY_ID"] != cred.AccessKeyID {
			t.Errorf("NewDefaultConf() %q != %s, error = %v", Expected["AWS_ACCESS_KEY_ID"], cred.AccessKeyID, err)
		}

		if Expected["AWS_SECRET_ACCESS_KEY"] != cred.SecretAccessKey {
			t.Errorf("NewDefaultConf() %q != %s, error = %v", Expected["AWS_SECRET_ACCESS_KEY"], cred.SecretAccessKey, err)
		}

		if Expected["AWS_SESSION_TOKEN"] != cred.SessionToken {
			t.Errorf("NewDefaultConf() %q != %s, error = %v", Expected["AWS_SESSION_TOKEN"], cred.SessionToken, err)
		}

		if Expected["AWS_REGION"] != gotCfg.Region {
			t.Errorf("NewDefaultConf() %q != %s, error = %v", Expected["AWS_REGION"], gotCfg.Region, err)
		}
	})
	...
	t.Run("using profile from env vars", func(t *testing.T) {
		...
		if Expected["AWS_SHARED_CREDENTIALS_FILE"] != cred.Source {
			t.Errorf("NewDefaultConf() %q != %s, error = %v", Expected["AWS_SHARED_CREDENTIALS_FILE"], cred.Source, err)
		}
